#!/usr/bin/env python3
"""
Interactive Rainbow Tunnel - Rotating tunnels with cyberpunk aesthetics
...
pygame.display.set_caption("◉ TUNNEL.EXE - CYBERPUNK MODE ◉")
...
class InteractiveTunnel:
    def __init__(self):
        # Tunnel parameters
        self.rotation_speed = 1.0
        self.color_cycle_speed = 1.0
        self.tunnel_depth = 1.0
        self.pattern_mode = 0  # 0: Classic, 1: Hexagon, 2: Square, 3: Spiral
    ...
    def get_color_palette(self, hue, saturation, value, time_val):
        ...
        if self.color_mode == 0:  # Cyberpunk
            ...
            g = int(255 * abs(math.sin((hue + 120) * math.pi / 180)))
            b = int(255 * abs(math.sin((hue + 240) * math.pi / 180)))
            ...
            r = int(255 * (0.8 + 0.2 * math.sin(hue * math.pi / 180)))
            g = int(100 * (0.5 + 0.5 * math.sin((hue + 90) * math.pi / 180)))
            b = int(255 * (0.9 + 0.1 * math.sin((hue + 180) * math.pi / 180)))
        else:  # Retro Future
    ...
    def calculate_tunnel_effect(self, x, y, time_val):
        """Calculate tunnel effect based on pattern mode"""
        # Center coordinates
        cx = x - WIDTH // 2
        cy = y - HEIGHT // 2
        ...
        if self.pattern_mode == 0:  # Classic
            if distance > 0:
                u = 32 / distance * self.tunnel_depth + time_val * self.rotation_speed * 0.5
                v = angle * 4 / math.pi + time_val * self.rotation_speed * 0.3
            else:
                u = 0
                v = 0
        elif self.pattern_mode == 1:  # Hexagon
            if distance > 0:
                # Create hexagonal pattern
                hex_angle = angle * 3 / math.pi
                u = 32 / distance * self.tunnel_depth + time_val * self.rotation_speed * 0.5
                v = hex_angle + time_val * self.rotation_speed * 0.3
                # Add hexagonal distortion
                u += 5 * math.sin(hex_angle * 6)
            else:
                u = 0
                v = 0
        elif self.pattern_mode == 2:  # Square
            if distance > 0:
                # Create square pattern
                square_dist = max(abs(cx), abs(cy))
                u = 32 / square_dist * self.tunnel_depth + time_val * self.rotation_speed * 0.5
                v = angle * 2 / math.pi + time_val * self.rotation_speed * 0.3
            else:
                u = 0
                v = 0
        else:  # Spiral
            if distance > 0:
                u = distance * 0.02 + time_val * self.rotation_speed * 0.5
                v = angle * 8 / math.pi + distance * 0.01 + time_val * self.rotation_speed * 0.3
            else:
                u = 0
                v = 0
        
        # Create rainbow effect with color cycling
        hue = (u * 50 + v * 80 + time_val * self.color_cycle_speed * 100) % 360
        saturation = 1.0
        value = 0.5 + 0.5 * math.sin(u * 2 + time_val * self.pulse_strength)
        
        return self.get_color_palette(hue, saturation, value, time_val)
    
    def draw_tunnel(self, surface, time_val):
        """Draw tunnel effect with pixel art style"""
        # Draw tunnel effect in pixel blocks
        for x in range(0, WIDTH, self.pixel_size):
            for y in range(0, HEIGHT, self.pixel_size):
                ...
                color = self.calculate_tunnel_effect(sample_x, sample_y, time_val)
                
                # Draw pixel block
                pygame.draw.rect(surface, color, (x, y, self.pixel_size, self.pixel_size))
    
    def draw_center_effects(self, surface, time_val):
        """Draw center effects and decorations"""
        center_x, center_y = WIDTH // 2, HEIGHT // 2
        
        # Pulsing center
        pulse_radius = int(10 + 5 * math.sin(time_val * 3 * self.pulse_strength))
        pulse_color = self.get_color_palette(time_val * 100 % 360, 1.0, 1.0, time_val)
        pygame.draw.circle(surface, pulse_color, (center_x, center_y), pulse_radius)
        
        # Rotating lines for extra effect
        num_lines = 8 if self.pattern_mode == 0 else 6 if self.pattern_mode == 1 else 4
        for i in range(num_lines):
            angle = time_val * 2 * self.rotation_speed + i * math.pi / (num_lines // 2)
            end_x = center_x + int(150 * math.cos(angle))
            end_y = center_y + int(150 * math.sin(angle))
            line_color = self.get_color_palette((time_val * 50 + i * 45) % 360, 0.8, 0.6, time_val)
            pygame.draw.line(surface, line_color, (center_x, center_y), (end_x, end_y), 2)
    
    def handle_input(self, keys):
        """Handle keyboard input for interactivity"""
        # Rotation speed controls
        if keys[pygame.K_UP]:
            self.rotation_speed = min(self.rotation_speed + 0.05, 3.0)
        if keys[pygame.K_DOWN]:
            self.rotation_speed = max(self.rotation_speed - 0.05, 0.1)
        
        # Color cycling speed controls
        if keys[pygame.K_LEFT]:
            self.color_cycle_speed = max(self.color_cycle_speed - 0.05, 0.1)
        if keys[pygame.K_RIGHT]:
            self.color_cycle_speed = min(self.color_cycle_speed + 0.05, 3.0)
        
        # Tunnel depth controls
        if keys[pygame.K_w]:
            self.tunnel_depth = min(self.tunnel_depth + 0.05, 2.0)
        if keys[pygame.K_s]:
            self.tunnel_depth = max(self.tunnel_depth - 0.05, 0.3)
        ...
        # Pulse strength controls
        if keys[pygame.K_PAGEUP]:
            self.pulse_strength = min(self.pulse_strength + 0.05, 2.0)
        if keys[pygame.K_PAGEDOWN]:
            self.pulse_strength = max(self.pulse_strength - 0.05, 0.0)
    ...
    def draw_ui(self, surface):
        ...
        self.ui.draw_cyber_panel(surface, panel_rect, "TUNNEL.CONTROL.SYSTEM")
        ...
        self.ui.draw_cyber_text(surface, f"ROT: {self.rotation_speed:.1f}", 
                               (x_offset + 210, y_offset - 2), NEON_GREEN, self.ui.font_tiny)
        y_offset += 20
        
        # Color cycle speed with progress bar
        col_rect = pygame.Rect(x_offset, y_offset, 200, 12)
        self.ui.draw_progress_bar(surface, col_rect, self.color_cycle_speed, 3.0, NEON_CYAN)
        self.ui.draw_cyber_text(surface, f"COL: {self.color_cycle_speed:.1f}", 
                               (x_offset + 210, y_offset - 2), NEON_CYAN, self.ui.font_tiny)
        y_offset += 20
        
        # Tunnel depth with progress bar
        depth_rect = pygame.Rect(x_offset, y_offset, 200, 12)
        self.ui.draw_progress_bar(surface, depth_rect, self.tunnel_depth, 2.0, NEON_PURPLE)
        self.ui.draw_cyber_text(surface, f"DEPTH: {self.tunnel_depth:.1f}", 
        ...
        controls = [
            "↑↓ ROTATION ◇ ←→ COLOR.CYCLE ◇ WS DEPTH ◇ AD PIXEL.SIZE",
            "+/- BRIGHTNESS ◇ PGUP/PGDN PULSE ◇ [P] PATTERN ◇ [C] COLOR",
...
def main():
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    tunnel.cycle_pattern_mode()
                elif event.key == pygame.K_c:
                    tunnel.cycle_color_mode()
        ...
        # Draw tunnel effect
        tunnel.draw_tunnel(screen, current_time)
        
        # Draw center effects
        tunnel.draw_center_effects(screen, current_time)
        
        # Draw CRT scanlines
        tunnel.ui.draw_scanlines(screen)
        
        # Draw UI if enabled
        if show_ui:
            tunnel.draw_ui(screen)
